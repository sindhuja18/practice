...
Performance = pd.read_sql('select b.brand_name as Brand_name, ps.colval_22 as RAM, s.score5 as Avg_score from product p join brand b on b.id=p.brand_id join product_spec_map ps on ps.product_id=p.id join product_score s on s.product_id=ps.product_id where s.score_type="Total score" and p.id<2000', con=connection)
Performance.RAM = Performance.RAM.str.extract('([0-9.]+)',flags=0).astype(float)
Performance = Performance.fillna(value=0)
html_avg = Performance.groupby(['Brand_name']).mean()
...
fp = open('pr_perf_average.html','w')
fp.write(cm_table)
